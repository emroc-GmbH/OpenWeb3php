...
class SolidityType
{
    ...
    public function encode(mixed $value, string $name):array|string
    {
        ...
        if(method_exists($this,'inputFormat')){
    ...
    public function decode($value, $offset, $name)
    {
        if ($this->isDynamicArray($name)) {
            ...
            return $this->polyOutputFormat($param, $name);
        }
    ...
    // Proxy for polymorphic call
    private function polyInputFormat(array $value, string $name):mixed{
        if(method_exists($this,'inputFormat')){
            return $this->inputFormat($value, $name);
        }
        return [];
    ...
    // Proxy for polymorphic call
    private function polyOutputFormat(string $param, string $name):mixed{
        if(method_exists($this,'outputFormat')){
